...
/*	
	Search space = intersection OA(S)
*/
...
__isl_give isl_basic_set *isl_set_coefficients(__isl_take isl_set *set)
{
	...
	printf("\n\t\t primal polyhedra: %s",isl_basic_set_to_str(set->p[0]));
	printf("\n\t\t Do you want to Under-Approximate? ");
	int ans;
	scanf("%d",&ans);
	if(ans)
	{
		char * str; 
		scanf("%s",str);
		coeff = isl_basic_set_coefficients(isl_basic_set_read_from_str(isl_basic_set_get_ctx(set->p[0]),str));
	}	
	else
		coeff = isl_basic_set_coefficients(isl_basic_set_copy(set->p[0]));
	printf("\n\t\t dual polyhedra: %s",isl_basic_set_to_str(coeff));
	// Over-approximate the schedule space
	//coeff = overapproximation(coeff);

	for (i = 1; i < set->n; ++i) {
		isl_basic_set *bset, *coeff_i;
		bset = isl_basic_set_copy(set->p[i]);
		printf("\n\t\t primal polyhedra: %s",isl_basic_set_to_str(bset));
		coeff_i = isl_basic_set_coefficients(bset);
		printf("\n\t\t dual polyhedra: %s",isl_basic_set_to_str(coeff_i));
		//coeff_i = overapproximation(coeff_i);
